...
document.querySelector('[data-tab="favourites"]')?.addEventListener('click', async function () {
    ...
    grid.innerHTML = products.map(p => {
        ...
        const salePrice = p.catalogSalePrice ?? p.price;
        const compareAtPrice = p.catalogCompareAtPrice;
        const price = compareAtPrice != null && salePrice < compareAtPrice
            ? `<span style="color:#e74c3c;font-weight:700;">${Number(salePrice).toFixed(2)} ${BYN_HTML}</span>
               <span style="text-decoration:line-through;color:#999;font-size:13px;margin-left:6px;">${Number(compareAtPrice).toFixed(2)} ${BYN_HTML}</span>`
            : `<span style="font-weight:700;">${Number(salePrice).toFixed(2)} ${BYN_HTML}</span>`;
